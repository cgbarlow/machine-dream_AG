/**
 * LLMCluster v3 - Scale-aware pattern diversity for 10x runs
 *
 * Improvements over v2:
 * - Explicit 1-indexed pattern numbering in all prompts (fixes P0 bug)
 * - Broader pattern definitions emphasizing REASONING APPROACHES over constraint variations
 * - Self-critique checks for pattern BREADTH, not just mutual exclusivity
 * - Better diversity at scale (371+ experiences)
 *
 * Inherits from v2:
 * - MUTUALLY EXCLUSIVE patterns during identification
 * - LLM self-critique step before categorization
 * - More demanding categorization prompt (most SPECIFIC pattern)
 * - Request 15-20 patterns initially for diversity
...
/**
 * Configuration options for LLMCluster v3
 */
export interface LLMClusterV3Config {
...
/**
 * LLMCluster v3 Algorithm
 *
 * Performance: <180s for 500 experiences
 * Approach: Scale-aware LLM-driven pattern identification with diversity breadth
 */
export class LLMClusterV3 extends BaseClusteringAlgorithm {
  private llmClient: ValidatedLLMClient;
  private config: LLMClusterV3Config;
  ...
  /**
   * Request more patterns initially for diversity
   */
  private readonly PATTERN_COUNT_MIN = 15;
  private readonly PATTERN_COUNT_MAX = 20;

  constructor(llmClient: ValidatedLLMClient, config?: Partial<LLMClusterV3Config>) {
    const metadata: AlgorithmMetadata = {
      name: 'LLMCluster',
      version: 3,
      identifier: 'llmclusterv3',
      description: 'Scale-aware LLM-driven pattern identification with diversity breadth and 1-indexed prompts',
   ...
   * v3 improvement: Emphasize reasoning APPROACHES over constraint variations
...
CRITICAL RULES:
1. Each pattern MUST be mutually exclusive from all others
2. Patterns should span DIFFERENT reasoning approaches (not just constraint variations)
3. If you have 5+ patterns that are all "constraint-based", consolidate them into fewer broader patterns
...
Think like a taxonomist creating a classification system where each item belongs to exactly ONE category.`;
  ...
  /**
   * Build prompt requiring MUTUALLY EXCLUSIVE patterns with diversity breadth
   * v3: Emphasizes reasoning approaches over constraint variations
   */
...
Before identifying patterns, consider these REASONING APPROACHES:
- Constraint-based: Uses row/column/box constraints together
- Elimination-based: Rules out impossible values
- Completion-based: Finds last remaining option
- Multi-constraint: Combines multiple constraint types

Ensure your patterns span DIFFERENT reasoning approaches, not just variations of the same technique.
Do NOT create separate patterns for "row constraint" vs "column constraint" vs "box constraint" -
these are the SAME technique applied to different units.
...
- ID (P1, P2, etc.) - IMPORTANT: Use 1-indexed numbering starting at P1. Never use P0.
- NAME: Concise name
...
Now identify ${targetCount} MUTUALLY EXCLUSIVE patterns (numbered P1 through P${targetCount}):`;
  }
...
⟦Ω:Task⟧{
  ...
  ;; V3: Ensure diversity across reasoning APPROACHES
  diversity≜∃p∈constraint_based∧∃p∈elimination_based∧∃p∈completion_based
}

⟦Σ:RequiredFields⟧{
  ;; CRITICAL: Pattern numbers are 1-indexed (P1, P2, P3...). Do NOT use P0.
...
⟦Ε:Output⟧{
   ...
   * v3 improvement: Checks for pattern BREADTH (different approaches), not just mutual exclusivity
  ...
  private async selfCritiquePatterns(
    ...
    if (this.aispMode === 'aisp-full') {
      ...
      systemPrompt = 'You are reviewing pattern definitions for quality. Be critical. Check for BREADTH across different reasoning approaches, not just mutual exclusivity.';
...
5. NEW: Do patterns span DIFFERENT reasoning APPROACHES (constraint-based, elimination-based, completion-based)?
   - If you have 5+ patterns that are all variations of "constraint" or "elimination", that's NOT enough diversity!

If patterns are NOT distinct enough OR lack diversity across approaches, provide a REVISED list.
If patterns ARE distinct AND span different approaches, respond with "PATTERNS_OK" on the first line.

For any revision, use the same format (remember: patterns are numbered P1 through PN, never P0):
PATTERN: P1
NAME: [name]
DESC: [description]
...
CRITICAL: Use pattern numbers 1 through ${patterns.length}. Never use 0. Pattern numbering starts at 1, not 0.`;
...
3. IMPORTANT: Use pattern numbers 1 through ${patterns.length}. Never use 0. Every experience fits SOME pattern.
...
⟦Ε:Output⟧{
  ...
  ;; CRITICAL: n ∈ {1,2,3,...} - never 0. Pattern numbering starts at 1.
...
⟦Ε:Output⟧{
  ...
  ;; CRITICAL: n ∈ {1,2,3,...,${patterns.length}} - never 0. Pattern numbering starts at 1.
  ;; ADR-015: Use concrete examples, not placeholders
  ∀n∈{1..${patterns.length}}
  ∀output:syntax∈AISP
  ¬prose
